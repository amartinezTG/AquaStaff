              ...
              <div class="card recent-sales overflow-auto">
                ...
                <div class="card-body">
                  ...
                  <form class="row g-3" method="POST" action="{{ route('submit_transfer') }}">
                    ...
                    <div class="row mb-12">
                      <!--<label for="inputEmail" class="col-sm-5 col-form-label">Agregar producto</label>
                      <input type="text" class="form-control" name="add_product" id="add_product" >
                      <div id="product-results"></div>-->
                    </div>
                    <div class="row mb-12">

                      <table class="table table-borderless " id="datatableRows">
                        <thead>
                          <tr>                          
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    $('#add_product').on('keyup', function() {
        ...
        $.ajax({
            url: "{{ route('search.products') }}", // Ruta a tu controlador
            data: { search: searchTerm },
            success: function(response) {
                // Limpiar los resultados anteriores
                $('#product-results').empty();

                // Crear una lista con los productos encontrados
                $.each(response.products, function(index, product) {
                    $('#product-results').append('<li>' + product.name + '</li>');
                });
            }
        });
    });
